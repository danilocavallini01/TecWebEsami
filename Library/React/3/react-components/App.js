      ...
      <div className="container">
        ...
        {this.state.griglia.map((riga, i) => {
          return <div key={i} class="riga">
            {riga.map((col, j) => {
              return <input type="text" class={col.cover ? "cover" : (col.value == "bomb" ? "bomb" : "") } key={i + j} row={i} col={j} value={col.value} readonly onClick={this.openCell} />
            })}
          </div>
        })
        }
